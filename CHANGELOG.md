...
- **Attribution learned which surface earned the impression.** Inventory now sells on three surfaces (the line, the page, the component), so one blended spend number was leaving yield unexplained. `SponsoredLogs.report` gains an `impressions_by_surface` breakdown and `report_text` a "By surface" footer, so you can see where the demand is landing. The count is per surface and bounded, the existing ledger snapshot is untouched (custom stores and caps ride through), and ActiveRecord books it in one migration
